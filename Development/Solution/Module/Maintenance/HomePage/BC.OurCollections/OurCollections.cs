...
{
    public class OurCollections : BaseModule
    {
        public override BaseModule Initialize(BrowserSession _Session, Pair _Parameter)
        {
            return new OurCollections(_Session, _Parameter);
        }

        public OurCollections()
        {

        }

        public OurCollections(BrowserSession _Session, Pair _Parameter)
        {
        ...
        public override ReturnSet LoadList()
        {
            try
            {
                using (var db = new OurCollectionsDB(Session))
                {
            ...
            catch (Exception ex)
            {
                ...
                Logger.LogError(ref message, "LoadListOurCollections", Helpers.CurrentUser(Session), "BC.OurCollections");
        ...
        public override ReturnSet LoadForm()
        {
            ...
            {
                string message = (ex.InnerException ?? ex).Message;
                Logger.LogError(ref message, "LoadFormOurCollections", Helpers.CurrentUser(Session), "BC.OurCollections");
                return new ReturnSet() { Message = message, Type = ReturnType.Error };
            }
        }
        ...
        public override ReturnSet Save()
        {
            using (var db = new OurCollectionsDB(Session))
            {
                ...
                {
                    string message = (ex.InnerException ?? ex).Message;
                    Logger.LogError(ref message, "SaveOurCollections", Helpers.CurrentUser(Session), "BC.OurCollections");
                    return new ReturnSet() { Message = message, Type = ReturnType.Error };
                }
            }
        }
        ...
        public override ReturnSet DeleteRecord()
        {
            ...
            {
                string message = (ex.InnerException ?? ex).Message;
                Logger.LogError(ref message, "DeleteRecordOurCollections", Helpers.CurrentUser(Session), "BC.OurCollections");
                return new ReturnSet() { Message = message, Type = ReturnType.Error };
            }
        }
        ...
        public virtual ReturnSet LoadCategory()
        {
            try
            {
                ...
                using (var db = new OurCollectionsDB(Session))
                {
                    var data = db.QueryTable<tCategory>("(SELECT * FROM dbo.tCategory WHERE IsActive = 1)a", to);
                    return new ReturnSet()
                    {
            ...
            {
                string message = (ex.InnerException ?? ex).Message;
                Logger.LogError(ref message, "LoadCategory", Helpers.CurrentUser(Session), "BC.OurCollections");
                return new ReturnSet() { Message = message, Type = ReturnType.Error };
            }
        }
        ...
        public override ReturnSet LoadDetail()
        {
            ...
            {
                string message = (ex.InnerException ?? ex).Message;
                Logger.LogError(ref message, "LoadOurCollectionsDetail", Helpers.CurrentUser(Session), "BC.OurCollections");
                return new ReturnSet() { Message = message, Type = ReturnType.Error };
            }
        }
    ...
    public class OurCollectionsDB : InSysContext
    {
        public OurCollectionsDB(BrowserSession Session) : base(Session)
        {
