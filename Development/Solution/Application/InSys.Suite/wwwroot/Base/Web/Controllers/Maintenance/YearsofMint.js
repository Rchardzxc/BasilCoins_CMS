angular.module('app')
    .controller('MaintenanceYearsofMint', ['$scope', '$controller', '$priv', '$state', function ($s, $c, $priv, $st) {
        ...
        $s.Init = function () {
            if ($s.RecordID != '') {
                ...
                $s.LoadTable($s.tblOptions, 'LoadList', 'YearsofMint', { MenuCode: 'MaintenanceYearsofMint' }).then(function (ret) {
                    if (ret.Type == 2) {
        ...
        $s.LoadForm = function () {
            return $s.Request('LoadForm', { ID: $s.RecordID, MenuCode: 'MaintenanceYearsofMint' }, 'YearsofMint');
        }

        $s.saveForm = function () {
            if ($s.IsTabsValid('form.yearsofmint')) {
                ...
                $s.Request('SaveForm', { Data: $s.Schema, MenuCode: 'MaintenanceYearsofMint' }, 'YearsofMint').then(function (ret) {
                    if (ret.Type == 2) {
                        $s.SetSystemStatus(ret.Message, 'error');
                        $s.$apply();
                    } else {
        ...
        $s.delete = function () {
            $s.deleteRow('DeleteRecord', 'YearsofMint', { MenuCode: 'MaintenanceYearsofMint' })
        }
