angular.module('app')
    .controller('MaintenanceColor', ['$scope', '$controller', '$priv', '$state', function ($s, $c, $priv, $st) {
        ...
        $s.Init = function () {
            if ($s.RecordID != '') {
                ...
                $s.LoadTable($s.tblOptions, 'LoadList', 'Color', { MenuCode: 'MaintenanceColor' }).then(function (ret) {
                    if (ret.Type == 2) {
        ...
        $s.LoadForm = function () {
            return $s.Request('LoadForm', { ID: $s.RecordID, MenuCode: 'MaintenanceColor' }, 'Color');
        }

        $s.saveForm = function () {
            if ($s.IsTabsValid('form.color')) {
                ...
                $s.Request('SaveForm', { Data: $s.Schema, MenuCode: 'MaintenanceColor' }, 'Color').then(function (ret) {
                    if (ret.Type == 2) {
                        $s.SetSystemStatus(ret.Message, 'error');
                        $s.$apply();
                    } else {
        ...
        $s.delete = function () {
            $s.deleteRow('DeleteRecord', 'Color', { MenuCode: 'MaintenanceColor' })
        }
