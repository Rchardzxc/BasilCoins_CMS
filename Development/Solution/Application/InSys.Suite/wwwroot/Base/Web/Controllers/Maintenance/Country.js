angular.module('app')
    .controller('MaintenanceCountry', ['$scope', '$controller', '$priv', '$state', function ($s, $c, $priv, $st) {
        ...
        $s.Init = function () {
            if ($s.RecordID != '') {
                ...
                $s.LoadTable($s.tblOptions, 'LoadList', 'Country', { MenuCode: 'MaintenanceCountry' }).then(function (ret) {
                    if (ret.Type == 2) {
        ...
        $s.LoadForm = function () {
            return $s.Request('LoadForm', { ID: $s.RecordID, MenuCode: 'MaintenanceCountry' }, 'Country');
        }

        $s.saveForm = function () {
            if ($s.IsTabsValid('form.country')) {
                ...
                $s.Request('SaveForm', { Data: $s.Schema, MenuCode: 'MaintenanceCountry' }, 'Country').then(function (ret) {
                    if (ret.Type == 2) {
                        $s.SetSystemStatus(ret.Message, 'error');
                        $s.$apply();
                    } else {
        ...
        $s.delete = function () {
            $s.deleteRow('DeleteRecord', 'Country', { MenuCode: 'MaintenanceCountry' })
        }
