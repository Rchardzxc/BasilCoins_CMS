angular.module('app')
    .controller('MaintenanceProductBrand', ['$scope', '$controller', '$priv', '$state', function ($s, $c, $priv, $st) {
        ...
        $s.Init = function () {
            if ($s.RecordID != '') {
                ...
                $s.LoadTable($s.tblOptions, 'LoadList', 'ProductBrand', { MenuCode: 'MaintenanceProductBrand' }).then(function (ret) {
                    if (ret.Type == 2) {
        ...
        $s.LoadForm = function () {
            return $s.Request('LoadForm', { ID: $s.RecordID, MenuCode: 'MaintenanceProductBrand' }, 'ProductBrand');
        }

        $s.saveForm = function () {
            if ($s.IsTabsValid('form.productbrand')) {
                ...
                $s.Request('SaveForm', { Data: $s.Schema, MenuCode: 'MaintenanceProductBrand' }, 'ProductBrand').then(function (ret) {
                    if (ret.Type == 2) {
                        $s.SetSystemStatus(ret.Message, 'error');
                        $s.$apply();
                    } else {
        ...
        $s.delete = function () {
            $s.deleteRow('DeleteRecord', 'ProductBrand', { MenuCode: 'MaintenanceProductBrand' })
        }
