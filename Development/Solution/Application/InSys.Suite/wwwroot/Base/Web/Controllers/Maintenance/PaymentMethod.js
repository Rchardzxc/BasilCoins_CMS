angular.module('app')
    .controller('MaintenancePaymentMethod', ['$scope', '$controller', '$priv', '$state', function ($s, $c, $priv, $st) {
        ...
        $s.Init = function () {
            if ($s.RecordID != '') {
                ...
                $s.LoadTable($s.tblOptions, 'LoadList', 'PaymentMethod', { MenuCode: 'MaintenancePaymentMethod' }).then(function (ret) {
                    if (ret.Type == 2) {
        ...
        $s.LoadForm = function () {
            return $s.Request('LoadForm', { ID: $s.RecordID, MenuCode: 'MaintenancePaymentMethod' }, 'PaymentMethod');
        }

        $s.saveForm = function () {
            if ($s.IsTabsValid('form.paymentmethod')) {
                ...
                $s.Request('SaveForm', { Data: $s.Schema, MenuCode: 'MaintenancePaymentMethod' }, 'PaymentMethod').then(function (ret) {
                    if (ret.Type == 2) {
                        $s.SetSystemStatus(ret.Message, 'error');
                        $s.$apply();
                    } else {
        ...
        $s.delete = function () {
            $s.deleteRow('DeleteRecord', 'PaymentMethod', { MenuCode: 'MaintenancePaymentMethod' })
        }
