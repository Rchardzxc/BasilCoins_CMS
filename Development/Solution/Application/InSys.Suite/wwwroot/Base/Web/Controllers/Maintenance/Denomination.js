angular.module('app')
    .controller('MaintenanceDenomination', ['$scope', '$controller', '$priv', '$state', function ($s, $c, $priv, $st) {
        ...
        $s.Init = function () {
            if ($s.RecordID != '') {
                ...
                $s.LoadTable($s.tblOptions, 'LoadList', 'Denomination', { MenuCode: 'MaintenanceDenomination' }).then(function (ret) {
                    if (ret.Type == 2) {
        ...
        $s.LoadForm = function () {
            return $s.Request('LoadForm', { ID: $s.RecordID, MenuCode: 'MaintenanceDenomination' }, 'Denomination');
        }

        $s.saveForm = function () {
            if ($s.IsTabsValid('form.denomination')) {
                ...
                $s.Request('SaveForm', { Data: $s.Schema, MenuCode: 'MaintenanceDenomination' }, 'Denomination').then(function (ret) {
                    if (ret.Type == 2) {
                        $s.SetSystemStatus(ret.Message, 'error');
                        $s.$apply();
                    } else {
        ...
        $s.delete = function () {
            $s.deleteRow('DeleteRecord', 'Denomination', { MenuCode: 'MaintenanceDenomination' })
        }
